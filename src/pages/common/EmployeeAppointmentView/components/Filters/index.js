...
const Filters = ({
    ...
    const handleSelectStatus = (e) => {
        if(e !== null && e !== undefined) 
            setValue('statusId', e.value, true);
    }
        ...
        <>
            ...
            onSubmit={handleSubmit((data) => {searchAppointments(data, setError)})}>
                <Row style={{'width':'100%', 'margin': '0', 'display': 'flex', 'justifyContent': 'center'}}>
                    <Col
                    xs={6}
                    md={(getLocalUser().roles.includes(ROLES.SUPERADMIN) || getLocalUser().roles.includes(ROLES.DENTIST)) ? 4 : 3} 
                    lg={(getLocalUser().roles.includes(ROLES.SUPERADMIN) || getLocalUser().roles.includes(ROLES.DENTIST)) ? 4 : 3} 
                    className='mb-2'>
                    ...
                    >
                        ...
                        options={ 
                            listStatus && [{value: 0, label: 'Todos'}, ...listStatus.map(data => {
                                return {
                                    value: data.id, 
                                    label: data.name
                                }
